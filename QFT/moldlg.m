 ...
 if (infmat(9,1)==1 | a==ap),
  ...
  if ~go_for_it,
   if lcont~=qflag,
    ...
    cp=ldlgcplx(cont(qflag,1),cont(qflag,2),w,T);
   end
   cont(qflag,1:4)=[cont(qflag,1)+delph,wval,NaN,5];
   cpnv=ldlgcplx(cont(qflag,1),cont(qflag,2),w,T);
   lo=lo.*(cpnv./cp);
   lomat(r,:)=lo;
   ...
   set(hint_bar,'string','Lead\Lag being implemented ...','fore','k');
