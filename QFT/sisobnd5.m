function bdb = sisobnd5(w,wbd,W,uP,vP,R,nom,uC,vC,loc,ph_r,info)
...
%                          |  1 + PGH  |
%
%     SISOBNDS(5,W,WBD,WS,P,R,NOM,C,LOC,PHS) computes bounds at frequencies
...
%
%     SISOBNDS(5,W,[],WS,P,[],[1,6],C,[],PHS) computes bounds using default
%     values for WBD (all frequencies in W), R (0), and LOC (1).
%     NOM specifies that the first row in P and the sixth row in C
%     are to be used as the nominal values.

% Author: Craig Borghesani
% Date: 9/6/93
% Revised: 2/16/96 11:23 AM V1.1 updates
...
% length(rp) == 1
  ...
  A=mC.^2 .*(mP.^2-(1 ./Ws+mP.*rad).^2); A=A(:,v);
...
bnd=[bmag; w(pbds); 5*ones(1,length(pbds))];

if any(R~=0),
 bndt=qrobust(w,w(pbds),1 ./R,uP,vP,0,nom,uC,vC,1,ph_r);
 [bnd,state2]=sectbnds([bnd,bndt]);
 if length(bnd), bnd(2*m+2,:)=5*ones(1,length(pbds)); end
end
...
if nargout==0,
 ...
 title('SISOBND5 Bounds'),xlabel('X: Phase (degrees)  Y: Magnitude (dB)')
else
 bdb=bnd(:,t);
end
