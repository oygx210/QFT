...
%                          |  1 + PGH  |
%
%     SISOBNDS(4,W,WBD,WS,P,R,NOM,C,LOC,PHS) computes bounds at frequencies
...
%
%     SISOBNDS(4,W,WBD,WS,P,1.2,[],[],2) computes bounds using default
%     values for NOM ([1,1]), C (1), and PHS ([0:-5:-360]).  With LOC=2,
%     the values in C are taken as G and bounds are computed for H.

% Author: Craig Borghesani
% Date: 9/6/93
% Revision: 2/16/96 11:21 AM V1.1 updates
...
% length(rp) == 1
  ...
  B=2*mP.*mC;
  if loc==1,
   A=mP.*mC.*(mP.*mC-2 ./Ws.*rad-mP.*mC.*rad.^2)-1 ./Ws.^2;
   B=B(:,v).*cos(psi);
   C=ones(lo2,m);
  else
   A=mP.^2 .*mC.^2 .*(1-rad.^2);
...
bnd=[bmag; w(pbds); 4*ones(1,length(pbds))];

if any(R~=0),
 bndt=qrobust(w,w(pbds),1 ./R,uP,vP,0,nom,uC,vC,1,ph_r);
 [bnd,state2]=sectbnds([bnd,bndt]);
 if length(bnd), bnd(2*m+2,:)=4*ones(1,length(pbds)); end
end
...
if nargout==0,
 ...
 title('SISOBND4 Bounds'),xlabel('X: Phase (degrees)  Y: Magnitude (dB)')
else
 bdb=bnd(:,t);
end
