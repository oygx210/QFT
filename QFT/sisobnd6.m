...
%                          |  1 + PGH  |
%
%     SISOBNDS(6,W,WBD,WS,P,R,NOM,C,LOC,PHS) computes bounds at frequencies
...
%
%     SISOBNDS(6,W,WBD,WS,P,[],[],[],[],PHS) computes bounds using default
%     values for R (0), NOM ([1,1]), C (1), and LOC (1).

% Author: Craig Borghesani
% Date: 9/6/93
% Revised: 2/16/96 11:25 AM V1.1 updates
...
% length(rp) == 1
  ...
  if loc==1,
   ...
   if length(lr),
    ...
    C(lr,:)=1-mP(lr,v).*mC(lr,v).*((1-rad(lr,v).^2)./Ws(lr,v).^2 .*mC(lr,v).*mP(lr,v)-...
       2*rad(lr,v)./Ws(lr,v));
   end
   if length(lr1),
    B(lr1,:)=mP(lr1,v).*mC(lr1,v).*cos(psi(lr1,:));
...
bnd=[bmag; w(pbds); 6*ones(1,length(pbds))];

if any(R~=0),
 bndt=qrobust(w,w(pbds),1 ./R,uP,vP,0,nom,uC,vC,1,ph_r);
 [bnd,state2]=sectbnds([bnd,bndt]);
 if length(bnd), bnd(2*m+2,:)=6*ones(1,length(pbds)); end
end
...
if nargout==0,
 ...
 title('SISOBND6 Bounds'),xlabel('X: Phase (degrees)  Y: Magnitude (dB)')
else
 bdb=bnd(:,t);
end
