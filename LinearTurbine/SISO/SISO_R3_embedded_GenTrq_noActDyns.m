...
%% Actuator TF
f_act = 0.03; wn_act = 2*pi*f_act; zeta_act = 0.7;
...
% --- Controller, G(s)
G_file  = fullfile( dirG, 'G_R3_embedded_GenTrq_noActDyns.shp' );
if( isfile(G_file) )
    G = getqft( G_file );
else
    syms s;
...
% --- Pre-filter file, F(s)
F_file  = fullfile( dirF, 'F_R3_embedded_GenTrq_noActDyns.fsh' );
if( isfile(F_file) )
    F = getqft( F_file );
else
